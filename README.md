...
- **Screen recording:** [docs/demo-video.mp4](docs/demo-video.mp4) — 60s walkthrough of the deployed app (Dashboard, Graph Explorer, Impact Analysis, Data Lineage, Sensitive-Data Exposure, Similar Agents, Similarity Leaderboard, Observability, global search, tech stack). Built with [Remotion](https://www.remotion.dev) from real screenshots of the live production deployment plus an original synthesized background score (`video/scripts/generate_bgm.py` — no third-party audio), source at [`video/`](video/), rather than a manual screen capture.
