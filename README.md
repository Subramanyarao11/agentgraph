...
- **Screen recording:** [watch on Google Drive](https://drive.google.com/file/d/1EeecyK9Xtx2BC-RBajWck6xbTjsuOrqn/view?usp=sharing) — 60s walkthrough of the deployed app (Dashboard, Graph Explorer, Impact Analysis, Data Lineage, Sensitive-Data Exposure, Similar Agents, Similarity Leaderboard, Observability, global search, tech stack). Also checked into the repo at [docs/demo-video.mp4](docs/demo-video.mp4) (GitHub's blob viewer doesn't reliably preview it — use the Drive link to watch). Built with [Remotion](https://www.remotion.dev) from real screenshots of the live production deployment plus an original synthesized background score (`video/scripts/generate_bgm.py` — no third-party audio), source at [`video/`](video/), rather than a manual screen capture.
